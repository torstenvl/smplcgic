...
/* cgi_init determines the data submission method, collects the data,
...
char *cgi_init(void)
{
    ...
    cgi_memAllocErr = '\0';        /* Error allocating memory */
    cgi_nullReqMeth = '\0';        /* REQUEST_METHOD not set/accessible */
    cgi_nullQryStrn = '\0';        /* QUERY_STRING not set/accessible */
    cgi_nullContLen = '\0';        /* CONTENT_LENGTH not set/accessible */
    cgi_usupReqMeth = '\0';        /* Unsupported request method */
    cgi_zeroContLen = '\0';        /* Zero content length (POST) */
    cgi_negMultIndx = '\0';        /* Negative index in multvalof */
    cgi_varNotFound = '\0';        /* Variable not in data string */
    cgi_nullArgPssd = '\0';        /* Null pointers passed to valof */

    if ((reqmeth = getenv("REQUEST_METHOD")) == NULL) {
        snprintf(cgi_err, CGIERRZ, "Request method not set--server error");
        return &cgi_nullReqMeth;
    }
    ...
    if (!strcmp(reqmeth, "POST")) {
        ...
        if ((scntlen = getenv("CONTENT_LENGTH")) == NULL) {
            snprintf(cgi_err, CGIERRZ, "Content length not set, though method is POST");
            return &cgi_nullContLen;
        }

        if ((cntlen = (size_t) strtoul(scntlen, (char **) NULL, 10)) == 0) {
            snprintf(cgi_err, CGIERRZ, "Content length is zero -- there is no data");
          return &cgi_zeroContLen;
        }

        if ((retstr = malloc(cntlen + 1)) == NULL) {
            snprintf(cgi_err, CGIERRZ, "Couldn't allocate enough memory even to store the submitted CGI data string");
            return &cgi_memAllocErr;
        }
    ...
    // FOR GET METHODS
        ...
        if ((qrystr = getenv("QUERY_STRING")) == NULL) {
            snprintf(cgi_err, CGIERRZ, "Query string not set, though method is GET");
            return &cgi_nullQryStrn;
        }
    ...
    // OTHERWISE IT'S AN ERROR
        ...
        snprintf(cgi_err, CGIERRZ, "The request method used is not supported");
        return &cgi_usupReqMeth;
    }
...
char *cgi_allvalof(char *valstr, char *s, int index)
{
    ...
    if ((valstr == NULL) || (s == NULL)) {
        snprintf(cgi_err, CGIERRZ, "NULL passed as an argument to cgi_allvalof()");
        return &cgi_nullArgPssd;
    }


    // SANITY: Check for negative index
    if (index < 0) {
        snprintf(cgi_err, CGIERRZ, "Cannot find a negative instance of something! Think!" );
        return &cgi_negMultIndx;
    }
    ...
    locinvalstr = (char *)&valstr[0];


    // Get the specific instance of the value that the user requested
    if (index > 0) {
    ...
    if (index == 0) {
        ...
        while (loc[0] != '\0') {
            ...
            if ((retstr = realloc(retstr, nstr_ln + 1)) == NULL) {
                snprintf(cgi_err, CGIERRZ, "Could not allocate enough memory to store the value of the variable");
                free(curvarval);
                free(retstr);
                return &cgi_memAllocErr;
            }
...
char *findvalue(char *valstr, char *s)
{
    ...
    if ((strncmp(valstr, s, s_ln) == 0) && (valstr[s_ln] == '=')) {
        ...
        if ((search = malloc(s_ln + 3)) == NULL) {
            snprintf(cgi_err, CGIERRZ, "Could not allocate enough memory to store the search string");
            return &cgi_memAllocErr;
        }
        sprintf(search, "&%s=", s);
        if ((loc = strstr(valstr, search)) == NULL) {
            ...
            free(search);
            snprintf(cgi_err, CGIERRZ, "Variable not found within the search string");
            return &cgi_varNotFound;
        }
